...
#define MENU_AUDIO_PATH "resources/ThemeSong.mp3"
...
void RenderGraphics(Uint32 *lastSpawnTime){
    ...
    //if(!app.GO){
        ...
        TTF_Font *font = TTF_OpenFont("resources/font.ttf", 24); // Load a font
        if(font == NULL) {
        ...
        char pointsText[100];
        sprintf(pointsText, "SCORE: %d", player1.points); // Convert points to string
        RenderText(pointsText, 10, 10, textColor, font); // Render text at position (10, 10)

        // Render ammo count
        char Ammocount[100];
        sprintf(Ammocount, "AMMO: %d", ammo ); // Convert points to string
        RenderText(Ammocount, 10, 35, textColor, font); // yaha pe position (10, 30)adjustable hai
...
void EnemyToPlayerDamage(void){
    ...
    for(int i = 0; i < MAX_ENEMIES; i++){
        ...
        if(checkCollision(playerRect, enemyRect)&&enemies[i].isAlive){
...
const Uint8* keystates=SDL_GetKeyboardState(NULL);
    ...
    char str[2]="F";
    RenderText(str,box.x+15,box.y-30,textColor,font);
...
void renderDoors(Uint32 *lastSpawnTime){
    ...
    if (player1.x <= 70 && player1.y >= (SCREEN_HEIGHT - DOOR2_HEIGHT) / 2 - 64 && player1.y <= (SCREEN_HEIGHT + DOOR2_HEIGHT + 64) / 2 && counter_x != -1) {
        RenderText(str1, 30,(SCREEN_HEIGHT - 20) / 2,textColor,font);
    ...
    if (player1.y <= 70 && player1.x >= (SCREEN_WIDTH - DOOR_WIDTH) / 2 - 64 && player1.x <= (SCREEN_WIDTH + DOOR_WIDTH + 64) / 2 && counter_y != -1) {
        RenderText(str1, (SCREEN_WIDTH - 20) / 2,DOOR_HEIGHT ,textColor,font);
    ...
    if (player1.x >= SCREEN_WIDTH - 2 * DOOR2_WIDTH - 70 && player1.y >= (SCREEN_HEIGHT - DOOR2_HEIGHT) / 2 - 64 && player1.y <= (SCREEN_HEIGHT + DOOR2_HEIGHT + 64) / 2 && counter_x != 1) {
        ...
        RenderText(str1,SCREEN_WIDTH - DOOR2_WIDTH - 20,(SCREEN_HEIGHT - 20) / 2 ,textColor,font);
        if (keystates[SDL_SCANCODE_G] && player1.points >= 1000) {
            player1.points -= 1000;
            counter_x = (counter_x + 1);
             counter_y = 0;
    ...
    if (player1.y >= SCREEN_HEIGHT - 2 * DOOR_HEIGHT - 70 && player1.x >= (SCREEN_WIDTH - DOOR_WIDTH) / 2 - 64 && player1.x <= (SCREEN_WIDTH + DOOR_WIDTH + 64) / 2 && counter_y != 1) {
        ...
        RenderText(str1,(SCREEN_WIDTH - 20) / 2,(SCREEN_HEIGHT - DOOR_HEIGHT - 20) ,textColor,font);
        if (keystates[SDL_SCANCODE_G] && player1.points >= 1000) {
            player1.points -= 1000;
            counter_y = (counter_y + 1);
            counter_x = 0;
...
void manageMenu(void){
    Mix_Music* menumusic=Mix_LoadMUS(MENU_AUDIO_PATH);
    if(!menumusic){
...
void handleGameOver(int *scene){
    Menu menu = {{"Restart", "Quit", "", "", "\0"}, {"resources/gameover.png"}, 0};
    int selectedBox = displayOptions(&menu,&done);
    switch (selectedBox) {
        case 0:
            app.menuState = 0;
